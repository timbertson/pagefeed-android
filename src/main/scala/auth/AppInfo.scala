...
class AppInfo extends Activity {
	...
	private class GetCookieTask extends AsyncTask[String, Void, Boolean] {
		protected def doInBackground(tokens: String*):Boolean = {
			try {
				// Don't follow redirects
				http_client.getParams().setBooleanParameter(ClientPNames.HANDLE_REDIRECTS, false)
				
				var http_get = new HttpGet("https://yourapp.appspot.com/_ah/login?continue=http://localhost/&auth=" + tokens(0))
				var response = http_client.execute(http_get)
				if(response.getStatusLine().getStatusCode() != 302)
					// Response should be a redirect
					return false
				
				if(http_client.getCookieStore().getCookies().exists { _.getName == "ACSID" }) {
					return true
				}
			} catch {
				case e: Exception => e.printStackTrace()
	...
	private class AuthenticatedRequestTask extends AsyncTask[String, Void, HttpResponse] {
		override protected def doInBackground(urls: String*):HttpResponse = {
			try {
				var http_get = new HttpGet(urls(0))
